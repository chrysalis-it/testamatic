...
export type ClientAndServer = { client: RestClient; server: http.Server; close: () => Promise<unknown> }
export type ClientAndServerProvider = <ENVKEYS>(env: EnvVars<ENVKEYS>) => Promise<ClientAndServer>
...
const defaultClientAndServerProvider =
  (config: ClientAndServerConfig): ClientAndServerProvider =>
    async <ENVKEYS>(env: EnvVars<ENVKEYS>): Promise<ClientAndServer> => {
...
const closeMaker = (client: RestClient, server: http.Server) => (): Promise<unknown> => {
  ...
  const closeClientPromise = Promise.resolve().then(() => client.client.dispose())
